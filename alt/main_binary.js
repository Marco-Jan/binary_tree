    ...
    recursivMethod(val) {
        const current = (node) => {
            if (node.value === val)
                return;
            if (node.value > val) {
                current((node.left = node.left ?? new TreeNode(val)));
            }
            if (node.value < val) { // Nullischer Koaleszenzoperator (??)
                current((node.right = node.right ?? new TreeNode(val)));
            }
        };
        current((this.root = this.root ?? new TreeNode(val)));
    }
    //find Method
    find(val) {
...
//# sourceMappingURL=main_binary.js.map